...
function initParticles() {
  ...
  for (let i = 0; i < 40; i++) {
    ...
    p.style.cssText = `width:${size}px;height:${size}px;left:${x}%;background:${color};animation-duration:${dur}s;animation-delay:${delay}s;`;
...
document.getElementById('searchInput')?.addEventListener('input', function() {
  ...
  const matches = [];
  GAMES.forEach(g => {
    if (g.name.toLowerCase().includes(q)) matches.push({ emoji: g.emoji, name: g.name, sub: g.sub, href: `game-${g.id}.html` });
  });
  ARTICLES.forEach(a => {
    if (a.title.toLowerCase().includes(q)) {
      const g = getGame(a.gameId);
      matches.push({ emoji: g ? g.emoji : '📄', name: a.title, sub: g ? g.name : '', href: `game-${a.gameId}.html` });
    }
  });

  results.innerHTML = matches.slice(0, 8).map(m => `
    <a href="${m.href}" class="search-result-item">
      <span class="sri-emoji">${m.emoji}</span>
      <div class="sri-info">
        <h4>${m.name}</h4>
        <span>${m.sub}</span>
      </div>
    </a>
  `).join('') || '<p style="color:var(--text3);padding:1rem 1.2rem;font-family:var(--font-ui)">Sin resultados para "' + q + '"</p>';
});
  ...
  grid.innerHTML = top.map(g => `
    <a href="game-${g.id}.html" class="game-card fade-in">
      <div class="gc-cover" style="background:linear-gradient(135deg,${g.color}22,${g.color}08)">
        <div class="gc-emoji-bg">${g.emoji}</div>
      ...
      <div class="gc-body">
        ...
        <div class="gc-vol">
          <span class="gc-vol-num">${fmtVol(g.vol)}</span>
          <span class="gc-vol-label">búsquedas/mes</span>
        </div>
...
function getTypeClass(t) {
  const m = {guia:'type-guia',trucos:'type-trucos',estrategia:'type-estrategia',secretos:'type-secretos',beginner:'type-beginner',tips:'type-tips',config:'type-config'};
  return m[t]||'type-tips';
}
function renderArticles() {
  ...
  grid.innerHTML = shown.map(a => {
    ...
    return `<div class="article-card fade-in" onclick="location.href='game-${a.gameId}.html'">
      <div class="ac-meta">
        <span class="ac-type ${getTypeClass(a.type)}">${typeLabel(a.type)}</span>
        <span class="ac-vol">🔍 ${fmtVol(a.vol)}/mes</span>
      </div>
      <h3 class="ac-title">${a.title}</h3>
      <div class="ac-game">
        <span class="ac-game-dot" style="background:${g?g.color:'#ccc'}"></span>
        <span>${g?g.name:'Varios'}</span>
      </div>
      <div class="ac-footer">
        <span class="ac-read">⏱ ${a.readTime} min de lectura</span>
        <span class="ac-arrow">→</span>
      </div>
    </div>`;
  }).join('');
  const loadMore = document.getElementById('loadMore');
  if (loadMore) loadMore.style.display = shown.length < filtered.length ? 'inline-block' : 'none';
  observeFadeIns();
}
renderArticles();
...
function renderBoardGames() {
  ...
  if (!grid) return;
  const mesaGames = GAMES.filter(g => g.cat === 'mesa').slice(0, 8);
  grid.innerHTML = mesaGames.map(g => `
    <a href="game-${g.id}.html" class="bg-card fade-in" style="border-color:${g.color}22">
      <div style="position:absolute;top:0;left:0;right:0;height:3px;background:${g.color};border-radius:12px 12px 0 0"></div>
      <div class="bg-emoji">${g.emoji}</div>
      <div class="bg-name">${g.name}</div>
      <div class="bg-sub">${g.sub}</div>
      <p class="bg-desc">${g.desc}</p>
      <div class="bg-stats">
        <div class="bg-stat">
          <div class="bg-stat-val" style="color:${g.color}">${fmtVol(g.vol)}</div>
          <div class="bg-stat-key">búsquedas</div>
        </div>
        <div class="bg-stat">
          <div class="bg-stat-val" style="color:${g.color}">${g.articles}</div>
          <div class="bg-stat-key">artículos</div>
        </div>
      </div>
    </a>
  `).join('');
